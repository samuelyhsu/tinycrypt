...
static rt_err_t test_tiny_aes(void) {
  ...
  tiny_aes_crypt_cbc(&ctx, AES_ENCRYPT, rt_strlen(data), iv, data,
                     data_encrypt);
  ...
  tiny_aes_crypt_cbc(&ctx, AES_DECRYPT, rt_strlen(data), iv, data_encrypt,
                     data_decrypt);
...
MSH_CMD_EXPORT_ALIAS(test_tiny_aes, tiny_aes_sample,
                     type tiny_aes_sample cmd to run);
